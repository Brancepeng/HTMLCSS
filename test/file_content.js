...
function runTests() {
    ...
    describe('Test if the files from the "' + dir + '" directory have the expected content', function () {
        ...
        it('"3d-album.html" should contain the correct jQuery version in the CDN URL', function (done) {
            var string = 'ajax.googleapis.com/ajax/libs/jquery/' + pkg.devDependencies.jquery + '/jquery.min.3d-album';
            checkString(path.resolve(dir, 'index.html'), string, done);
        });

        it('"3d-album.html" should contain the correct jQuery version in the local URL', function (done) {
            var string = '3d-album/vendor/jquery-' + pkg.devDependencies.jquery + '.min.3d-album';
            checkString(path.resolve(dir, 'index.html'), string, done);
        });
